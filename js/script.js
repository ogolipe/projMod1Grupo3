...
modelsJson.map((item, index)=>{
    ...
    modelsItem.querySelector('a').addEventListener('click', (e)=>{
        ...
        cs('.modelsInfo--size').forEach((size, sizeIndex)=>{
            ...
            //size.querySelector('span').innerHTML = modelsJson[key].sizes[sizeIndex];/*  Colocar o tamanho e ML */
        });
...
//Ações do modal - janela quando fechar
function closeModal() {
    c('.modelsWindowArea').style.opacity = 0;
    setTimeout(()=>{
...
//Ações do CANCELAR e SAIR(MOBILE)
